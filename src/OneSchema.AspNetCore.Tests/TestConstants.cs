...
internal static class TestConstants
{
    ...
    public static string GenerateValidJwt(string userId = "user-123")
        => OneSchemaJwt.GenerateEmbedToken(ClientId, ClientSecret, userId);

    public static string GenerateJwtWithWrongSecret(string userId = "user-123")
        => OneSchemaJwt.GenerateEmbedToken(ClientId, "wrong-secret-key-that-is-long-enough-for-hmac", userId);
}
